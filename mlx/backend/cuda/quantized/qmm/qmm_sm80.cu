...
void qmm_sm80(
  ...
  auto n_residue = n % cute::size<1>(cta_tiler) != 0;
  std::string module_name = fmt::format(
      "qmm_sm80_ss{}tn_{}_m{}_b{}_g{}_{}",
      n_residue ? "_nres_" : "_",
  ...
  size_t scale_elements = cute::size<1>(cta_tiler) *
      (cute::size<2>(cta_tiler) / group_size) * cute::size<2>(sA_layout);
  size_t scale_bytes = scale_elements * (scales.itemsize() + x.itemsize());
  size_t smem_bytes = std::max(
      cute::cosize(sA_layout) * x.itemsize() +
          cute::cosize(sB_layout) * bits / 8 + scale_bytes,
...
bool supports_gather_qmm_rhs_sm80(
  ...
  return rows >= 16 && group_count > 0 && rows / group_count >= 4;
}
...
void gather_qmm_rhs_sm80(
  ...
  std::string module_name = fmt::format(
      "gather_qmm_rhs_sm80_ss_tn_{}_m{}_b{}_g{}_{}",
      dtype_to_string(x.dtype()),
      cute::size<0>(cta_tiler),
      bits,
      group_size,
